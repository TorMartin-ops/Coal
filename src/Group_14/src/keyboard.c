...
//============================================================================
#include "keyboard.h"
#include "keyboard_hw.h" // Should define KB_RESP_ACK, etc.
#include "types.h"
#include "idt.h"          // For PIC_EOI, outb, PIC1_COMMAND, PIC2_COMMAND constants
#include "port_io.h"      // For outb, inb
#include <isr_frame.h>
#include "terminal.h"
#include "pit.h"          // For get_pit_ticks()
#include "string.h"
#include "serial.h"
#include "spinlock.h"
#include "assert.h"
...
void keyboard_init(void) {
    ...
    terminal_write("  [KB Init] Default US keymap (Set 1 style) loaded. Device will use Set 2. KBC Translation will be ON (config 0x41).\n");
    ...
    terminal_printf("  [KB Init] Config Byte after KB Enable cmd (0xAE) = 0x%02x\n", config_after_kb_enable);
    ...
    if (!kbc_expect_ack("KB Device Reset (0xFF)")) {
        terminal_write("  [KB Init] Warning: No ACK for KB Device Reset command. Keyboard may not respond.\n");
    } else {
    ...
    if (kbc_expect_ack("Set Scancode-Set CMD (0xF0)")) {
        ...
        if (!kbc_expect_ack("Set Scancode-Set DATA (0x02)")) {
            terminal_write("  [KB Init] Warning: Keyboard did not ACK scancode-set data byte (0x02) or requested RESEND.\n");
            // You could add retry logic here if resp was KB_RESP_RESEND
        }
        terminal_write("  [KB Init] Scancode-Set #2 selected (commands sent).\n");
    } else {
        terminal_write("  [KB Init] WARNING: Keyboard did not ACK scancode-set command (0xF0), leaving default.\n");
    }
    ...
    if (!kbc_expect_ack("Enable Scanning (0xF4)")) {
         terminal_write("  [KB Init] Warning: Failed to get ACK for Enable Scanning! Keyboard might not send keys.\n");
    } else {
